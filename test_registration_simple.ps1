...
if (-not $foundFilters) {
    Write-Host "   No SpoutCam DirectShow filters found" -ForegroundColor Red
}
...
if (Test-Path $dllPath32) {
    ...
    Write-Host "   SpoutCam32.ax not found" -ForegroundColor Yellow
}
...
if (Test-Path $dynamicCameraPath) {
    ...
    if ($cameras) {
        ...
        Write-Host "   Dynamic camera registry path exists but no cameras found" -ForegroundColor Yellow
    }
} else {
