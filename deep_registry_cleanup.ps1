...
Write-Host "`n=== Phase 1: User Registry Cleanup ===" -ForegroundColor Cyan
...
Write-Host "`n=== Phase 3: DirectShow Filter Cleanup ===" -ForegroundColor Cyan
...
Write-Host "`n=== Phase 4: Search for Hidden SpoutCam References ===" -ForegroundColor Cyan
...
Write-Host "`n=== Phase 5: COM Interface Cleanup ===" -ForegroundColor Cyan
...
if (-not $hasRemainingEntries) {
    ...
    Write-Host "`n   Some entries may require:" -ForegroundColor Yellow
...
Write-Host "`n=== Registry Backup Recommendation ===" -ForegroundColor Cyan
...
Write-Host "   reg export `"HKEY_CURRENT_USER\Software\Leading Edge`" `"$backupPath`"" -ForegroundColor Gray
